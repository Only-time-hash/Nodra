...
create or replace function public.consume_gateway_rate_limit(
  p_workspace_id uuid, p_agent_id uuid, p_limit integer default 240, p_window_seconds integer default 60
...
begin
  ...
  if p_limit < 1 or p_limit > 10000 or p_window_seconds < 1 or p_window_seconds > 3600
    then raise exception 'invalid_rate_limit_parameters'; end if;
  ...
  return query select v_count<=p_limit, greatest(p_limit-v_count,0),
    greatest(1,ceil(extract(epoch from (v_bucket+make_interval(secs=>p_window_seconds)-v_now)))::integer);
end $$;
revoke all on function public.consume_gateway_rate_limit(uuid,uuid,integer,integer) from public, anon;
grant execute on function public.consume_gateway_rate_limit(uuid,uuid,integer,integer) to authenticated, service_role;
