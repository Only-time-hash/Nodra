...
begin
  ...
  where i.id=p_incident_id and i.state='recovering'
  for update;
  ...
  where rp.incident_id=p_incident_id and rp.workspace_id=v_workspace
  for update;
  if v_plan is null then return false; end if;

  select count(*) into v_pending
  from public.recovery_steps rs
  where rs.recovery_plan_id=v_plan and rs.status::text <> 'ready';
  if v_pending<>0 then return false; end if;

  if not (
    coalesce((v_checks->>'originPatched')::boolean,false) and
    coalesce((v_checks->>'credentialsRotated')::boolean,false) and
    coalesce((v_checks->>'memoryReviewed')::boolean,false) and
    coalesce((v_checks->>'pendingJobsReviewed')::boolean,false) and
    coalesce((v_checks->>'humanApproved')::boolean,false)
  ...
  update public.agents
    ...
    where workspace_id=v_workspace
      and kind='laboratory'
      and status in ('quarantined','at_risk');
  update public.incidents
    set state='resolved',resolved_at=now()
    where id=p_incident_id and workspace_id=v_workspace and state='recovering';
