...
begin
  ...
  if p_phase='intent' then
    ...
    on conflict on constraint gateway_outbox_workspace_id_request_id_phase_key
...
revoke all on function public.record_integration_gateway_event(text,text,text,timestamptz,timestamptz,text,text,text,text,text,boolean,text,text,text,uuid,uuid,timestamptz) from public,authenticated;
