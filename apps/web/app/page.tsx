...
export default function HomePage(){return <main className="refHome" id="top">
  ...
  <section className="refTeams refShell" id="solutions">
    <div className="teamsIntro"><span className="refEyebrow"><i/> BUILT FOR OPERATORS</span><h2>Control for the teams<br/><em>responsible when agents act.</em></h2><p>Security teams need visibility and intervention. Organizations need enforceable boundaries and accountable operations. Nodra brings both into one control plane.</p></div>
    <div className="teamPanels">
      <article className="teamPanel securityTeam"><div className="teamPanelTop"><span><ShieldAlert/></span><small>SECURITY OPERATIONS</small></div><h3>See risk before it becomes impact.</h3><p>Investigate protected runtime activity, hold sensitive actions for review, contain incidents and preserve the evidence needed to reconstruct what happened.</p><div className="teamChecks"><span><i/> Human approval gates</span><span><i/> Incident containment</span><span><i/> Causal investigation</span><span><i/> Tamper-evident activity</span></div><a className="teamAction" href="#security">Explore the security model <ArrowRight/></a></article>
      <article className="teamPanel orgTeam"><div className="teamPanelTop"><span><Building2/></span><small>ORGANIZATION CONTROL</small></div><h3>Give every agent an explicit operating boundary.</h3><p>Separate workspaces, bind runtime identities, scope authority and keep high-impact actions subject to deterministic controls instead of model judgment alone.</p><div className="teamChecks"><span><i/> Workspace isolation</span><span><i/> Explicit authority</span><span><i/> Role-aware administration</span><span><i/> Controlled recovery</span></div><GitHubAuthButton className="teamAction">Set up a workspace <ArrowRight/></GitHubAuthButton></article>
    </div>
    <aside className="securitySpine"><div className="spineMark"><ShieldCheck/><i/></div><div><small>SECURITY SPINE</small><h3>Identity → Authority → Decision → Evidence → Response</h3><p>The same boundary that authorizes an action also gives operators the context to investigate, contain and recover.</p></div><div className="spineSignals"><span><LockKeyhole/> Least privilege</span><span><Users/> Human oversight</span><span><Database/> Evidence chain</span></div><a href="/docs">Security model <ArrowRight/></a></aside>
  </section>
