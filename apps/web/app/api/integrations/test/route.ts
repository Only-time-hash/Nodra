...
export async function POST(request:Request){
 ...
 const {data:credential}=await ctx.supabase.from("integration_credentials").select("id,last_used_at,status").eq("workspace_id",ctx.workspaceId).eq("agent_id",agent.id).eq("status","active").order("created_at",{ascending:false}).limit(1).maybeSingle();
