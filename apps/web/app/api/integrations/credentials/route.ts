...
export async function POST(request:Request){
 ...
 const {data,error}=await ctx.supabase.from("integration_credentials").insert({workspace_id:ctx.workspaceId,agent_id:agent.id,label:String(body.label||"Default integration").slice(0,80),secret_hash:issued.hash,secret_prefix:issued.prefix,created_by:ctx.userId}).select("id,label,secret_prefix,status,created_at").single();
...
export async function DELETE(request:Request){
 ...
 if(!["owner","admin"].includes(ctx.role))return NextResponse.json({error:"insufficient_role"},{status:403});
 let body:any;try{body=await request.json()}catch{return NextResponse.json({error:"invalid_request"},{status:400})}
 const {data,error}=await ctx.supabase.from("integration_credentials").update({status:"revoked",revoked_at:new Date().toISOString()}).eq("workspace_id",ctx.workspaceId).eq("id",body?.credentialId).eq("status","active").select("id,status,revoked_at").maybeSingle();
...
export async function PUT(request:Request){
 ...
 const {data:old}=await ctx.supabase.from("integration_credentials").select("id,agent_id,label,status").eq("workspace_id",ctx.workspaceId).eq("id",body.credentialId).eq("status","active").maybeSingle();
 if(!old)return NextResponse.json({error:"credential_not_found"},{status:404});
 const issued=issueIntegrationSecret();
 const {data:next,error}=await ctx.supabase.from("integration_credentials").insert({workspace_id:ctx.workspaceId,agent_id:old.agent_id,label:old.label,secret_hash:issued.hash,secret_prefix:issued.prefix,created_by:ctx.userId,rotated_from:old.id}).select("id,label,secret_prefix,status,created_at").single();
 if(error||!next)return NextResponse.json({error:"credential_rotation_failed"},{status:500});
 const {error:revokeError}=await ctx.supabase.from("integration_credentials").update({status:"revoked",revoked_at:new Date().toISOString()}).eq("workspace_id",ctx.workspaceId).eq("id",old.id).eq("status","active");
 if(revokeError){await ctx.supabase.from("integration_credentials").delete().eq("workspace_id",ctx.workspaceId).eq("id",next.id);return NextResponse.json({error:"credential_rotation_failed"},{status:500})}
