...
const event={id:"evt-1",agentId:"research",resourceId:"browser",action:"read",decision:"allow" as const,timestamp:new Date().toISOString()};
...
test("flight recorder rejection fails closed", async()=>{
  ...
  try{
    ...
    await assert.rejects(()=>observer(event),/flight recorder rejected gateway event: 503/);
...
test("flight recorder network outage fails closed", async()=>{
  ...
  try{
    const observer=createFlightRecorderObserver({endpoint:"https://recorder.invalid/events",signingIdentity:identity});
    await assert.rejects(()=>observer(event),/network unavailable/);
  } finally { globalThis.fetch=originalFetch; }
});
