...
- [ ] Release candidate receives an independent verification pass against the assurance invariants in docs/v0.1.md. The final pass explicitly exercises OWASP agentic abuse cases: per-request authorization and tenant isolation, least-privilege tool scopes, human approval for high-impact actions, signed inter-agent identity, resource budgets/rate limits, and fail-closed handling of untrusted model/tool output.
