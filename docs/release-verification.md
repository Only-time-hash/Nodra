...
- [x] Atomic restart negative-path production assurance: transactional production-schema attacks verified restart is denied while any recovery step remains pending and is still denied after remediation completes if explicit human approval is absent. In both denial cases the incident remained recovering and the affected agent remained quarantined; the fixture was rolled back.

- [x] Continuous compromise-to-recovery production-schema lifecycle: authenticated Owner exercised blast-radius/containment scope (Manager quarantine; Research authority restriction), all four atomic remediation actions, adapter-backed recovery evidence, completed recovery steps, explicit human approval, and atomic restart. Restart returned true; incident resolved; Manager and Research returned healthy with authority scopes preserved; unrelated paused Support remained paused. Fixture rolled back. This closes the defined #35.16 lifecycle verification.

- [x] Final remediation/restart authorization regression: production permissions expose only the role-checked atomic remediation and restart entrypoints to authenticated users; remediation helpers and the legacy restart assessor remain service-role only. A transactional vertical-privilege attack confirmed a workspace Viewer is denied atomic remediation. Fixture rolled back. This closes the currently discovered #35.17 security regressions.

- [ ] #35.18 release-candidate deployment reconciliation: GitHub main is now `70dea0206e94c4e5b0aea3026114df73f4760f8c`, while the newest READY Vercel production deployment still targets `00196b8b00ae01a83b6aa90aa50bd96bc3569706`. Therefore the current production alias is stale relative to the security/recovery candidate and must not be used as evidence for the final tag. A direct connected deployment attempt was unavailable, so this remains an explicit release blocker rather than being silently marked green.

- [x] #35.18 historical production runtime-error reconciliation: the 7-day production error inventory contains five known historical groups (missing Supabase middleware configuration, Gemini 404, Flight Recorder 503/500, incident recorder permission denial, and relative gateway URL parsing), each tied to older deployments from September 19. No evidence from this inventory is attributed to the current undeployed release candidate. These remain regression targets after the newest main is deployed; they are not treated as proof that the current candidate is live or healthy.
